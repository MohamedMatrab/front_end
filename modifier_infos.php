...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['modifier'])) {
        ...
        if ($_FILES['image']['size'] > 0) {
            ...
            $query = $connect->prepare("SELECT img FROM users WHERE id=:id");
        ...
        $query = $connect->prepare("UPDATE users SET fname=:fname, lname=:lname, email=:email, cin=:cin, img=:image, phone_num=:phone WHERE id=:id");
